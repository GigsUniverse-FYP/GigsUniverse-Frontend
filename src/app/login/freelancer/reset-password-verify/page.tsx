...
export default function PasswordResetVerify() {
  ...
  const validateCode = (code: string) => {
    ...
    try {
        const response = await fetch(`${backendURL}/api/auth/freelancer/verify-reset-code`, {
        method: "POST",
        headers: {
        ...
        router.push(`/login/freelancer/reset-password?email=${encodeURIComponent(email)}&code=${encodeURIComponent(code)}`);
    ...
    try {
        const response = await fetch(`${backendURL}/api/auth/freelancer/resend-reset-code`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
