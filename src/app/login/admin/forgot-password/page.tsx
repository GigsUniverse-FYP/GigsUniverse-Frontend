  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const checkRes = await fetch(
        `${backendURL}/api/auth/admin/check-email?email=${encodeURIComponent(email)}`
      )
      ...
      const forgotRes = await fetch(`${backendURL}/api/auth/admin/forgot-password`, {
        method: "POST",
        headers: {
      ...
      router.push(`/login/admin/reset-password-verify?email=${encodeURIComponent(email)}`)
          ...
          <Card className="border-2 border-gray-300 bg-white shadow-lg">
            ...
            <CardContent>
              ...
              <div className="mt-6 text-center text-sm text-gray-600">
                ...
                <Link href="/login/admin" className="text-gray-900 hover:text-black font-medium">
                  Back to Login
                </Link>
              </div>
            </CardContent>
