...
export default function PasswordResetVerify() {
  ...
  const validateCode = (code: string) => {
    ...
    try {
        const response = await fetch(`${backendURL}/api/auth/employer/verify-reset-code`, {
        method: "POST",
        headers: {
        ...
        router.push(`/login/employer/reset-password?email=${encodeURIComponent(email)}&code=${encodeURIComponent(code)}`);
    ...
    try {
        const response = await fetch(`${backendURL}/api/auth/employer/resend-reset-code`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
