...
export default function Step1SumsubVerification() {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      try {
        const res = await fetch(`${backendUrl}/api/sumsub/freelancer/status`, {
          credentials: "include",
          method: "GET",
        });
  ...
  // Start verification is freelancer is not verified 
  const startVerification = async () => {
    try {
      const res = await fetch(`${backendUrl}/api/sumsub/freelancer/permalink`, {
        credentials: "include",
      });
