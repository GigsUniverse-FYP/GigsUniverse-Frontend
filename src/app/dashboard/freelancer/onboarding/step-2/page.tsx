...
export default function Step2ProfileCreation() {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        const res = await fetch(`${backendUrl}/api/profile/freelancer/session`, {
          credentials: "include",
        })
        ...
        setFormData((prev) => ({
          ...prev,
          freelancerProfileId: data.userId,
          email: data.email,
        }))
      } catch (err) {
        console.error(err)
  ...
  const handleFileChange = (
    ...
    if (multiple) {
      ...
      if (field === "portfolioFiles" || field === "certificationFiles") {
        currentFiles = formData[field as "portfolioFiles" | "certificationFiles"];
      }
  ...
  const addJobCategory = (category: string) => {
    if (formData.jobCategory.length < 3 && !formData.jobCategory.includes(category)) {
      handleInputChange("jobCategory", [...formData.jobCategory, category])
    }
  }
  ...
  const removeJobCategory = (category: string) => {
    handleInputChange(
      "jobCategory",
      formData.jobCategory.filter((c) => c !== category),
    )
  }
  ...
  const isFormValid = () => {
    ...
    const payRate = Number.parseFloat(formData.preferredPayRate)
  ...
  const saveToDatabase = async () => {
    ...
    try {
      ...
      formDataToSend.append('data', JSON.stringify(nonFileData)); // No Blob
        ...
        // Send to backend
        const response = await fetch(`${backendUrl}/api/profile/freelancer/save`, {
          method: 'POST',
          body: formDataToSend,
          credentials: 'include',
        });
        ...
        if (response.ok) {
          ...
          router.push("/dashboard/freelancer/onboarding/step-3");
        }
          ...
          <CardContent className="p-8">
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <Label htmlFor="freelancerProfileId" className="mb-2 block">
                    Freelancer Profile ID
                  </Label>
                  <Input
                    id="freelancerProfileId"
                    value={formData.freelancerProfileId}
                    disabled
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <Label htmlFor="highestEducationLevel" className="mb-2 block">
                    Highest Education Level *
                  </Label>
                  <Select
                    onValueChange={(value) => handleInputChange("highestEducationLevel", value)}
                ...
                <div>
                  <Label htmlFor="hoursPerWeek" className="mb-2 block">
                    Hours Per Week * (1-40)
                  </Label>
                  <Input
                    id="hoursPerWeek"
              ...
              <div className="w-full">
                <Label className="mb-2 block">Job Categories * (Max 3)</Label>
                <Select onValueChange={addJobCategory} disabled={formData.jobCategory.length >= 3}>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder="Select job categories" />
                  </SelectTrigger>
                  <SelectContent>
                    {jobCategories
                      .filter((category) => !formData.jobCategory.includes(category))
                      .map((category) => (
              ...
              <div className="w-full">
                ...
                <Select onValueChange={addSkillTag} disabled={formData.skillTags.length >= 10}>
                  <SelectTrigger className="w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <Label htmlFor="preferredPayRate" className="mb-2 block">
                    Preferred Pay Rate (Per Hour) * ($1-$150)
                  </Label>
                  <Input
                    id="preferredPayRate"
                    type="number"
                    value={formData.preferredPayRate}
                    onChange={(e) => handleInputChange("preferredPayRate", e.target.value)}
                    min={1}
                    max={150}
                    placeholder="50"
                    required
              ...
              {/* Resume */}
              <div>
                <Label htmlFor="resumeFile" className="mb-2 block">
                  Resume (Optional, Max 1 File, 10MB)
                </Label>
                <Input
                  id="resumeFile"
                  type="file"
                ...
                {formData.resumeFile && (
                  <div className="mt-4 p-4 border rounded-lg bg-gray-50">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => handleInputChange("resumeFile", null)}>
                        <XCircle className="w-4 h-4 text-red-500" />
                      </Button>
                    </div>
                  </div>
              ...
              {/* Portfolio */}
              <div className="space-y-4 border p-6 rounded-lg bg-white">
                <h3 className="text-lg font-semibold text-gray-900">Portfolio (Optional, Max 2 files, 10MB each)</h3>
                <Input
                  type="file"
                  multiple
                  accept="image/*,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                  ...
                  {formData.portfolioFiles.map((file, index) => (
                    <div key={index} className="p-4 border rounded-lg bg-gray-50">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => removeFile("portfolioFiles", index)}>
                          <XCircle className="w-4 h-4 text-red-500" />
                        </Button>
                      </div>
                    </div>
