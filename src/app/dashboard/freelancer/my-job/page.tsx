...
export default function HiredFreelancersPage() {
  ...
  useEffect(() => {
    const fetchContracts = async () => {
      try {
        const res = await fetch(`${backendUrl}/api/hired-freelancers/freelancer-view`, {
          method: "GET",
          credentials: "include",
        });
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="flex-1">
                  <h3 className="font-bold text-gray-900">{freelancer.contract.jobName}</h3>
                </div>
              </div>
              ...
              <div className="space-y-3 mb-4">
                <div>
                  <p className="text-sm font-medium text-gray-900 mb-2">{freelancer.name}</p>
                ...
                <div className="text-xs text-gray-600 space-y-1">
                  ...
                  <div className="flex justify-between">
                    <span>Start:</span>
                    <span className="font-medium">{new Date(freelancer.contract.startDate).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>End:</span>
                    <span className="font-medium">{new Date(freelancer.contract.endDate).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
