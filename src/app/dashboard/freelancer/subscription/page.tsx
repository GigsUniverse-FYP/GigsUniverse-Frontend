...
import { CheckCircle, HelpCircle, CreditCard, Shield, Clock, FileText, DollarSign, Calendar, AlarmCheckIcon, CheckCircleIcon } from "lucide-react"
...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    if (showBillingHistory) {
      ...
      fetch(`${backendUrl}/api/freelancer/subscription/bill-data`, {
        method: "GET",
        headers: {
  ...
  const purchaseSubscription = async () => {
    const res = await fetch(`${backendUrl}/api/freelancer/subscription/purchase`, {
      method: "POST",
      credentials: "include",
    });
  ...
  const cancelSubscription = async () => {
    const res = await fetch(`${backendUrl}/api/freelancer/subscription/cancel/immediate`, {
      method: "POST",
      credentials: "include",
    });
  ...
  const handlePlanAction = async (planName: string) => {
    if (planName === "Premium Freelancer") {
      try {
        await purchaseSubscription()
      } catch (error) {
        console.error("Upgrade failed", error)
      }
    } else if (planName === "Free" && currentPlan === "Premium Freelancer") {
      try {
  ...
  useEffect(() => {
    const fetchPremiumStatus = async () => {
      try {
        ...
        setCurrentPlan(data ? "Premium Freelancer" : "Free")
      } catch (error) {
        console.error(error)
        setPremium(null)
        setCurrentPlan("Free")
