...
export default function ConfirmEmail() {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        const res = await fetch(`${backendURL}/api/auth/freelancer/confirm-email?token=${token}`)
        if (!res.ok) throw new Error("Verification failed")
        router.replace("/register/freelancer/email-success")
      } catch (err) {
        router.replace(`/register/freelancer/email-fail?email=${email}&token=${token}`)
      }
    }
